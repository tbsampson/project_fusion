...
SELECT TTL.SZEDUS
	...
	,TTL.SZBOSTR
FROM (
	SELECT DISTINCT 'JDE' SZEDUS /* EDI - User ID String Generic Edit(10)*/
		,'BALBOM' SZEDBT /*				SZEDTN -- EDI - Transaction Number String Generic Edit(22)*/
		...
		,IM1.SZITM SZKIT /* Parent (short) Item Number Numeric Generic Edit(8)*/
		,REQUIREMENT.WORKORDER_BASE_ID /* IM1.SZLITM   */ SZKITL /* Kit - 2nd Item Number String Generic Edit(25)*/
		,IM1.SZAITM /*    -- SZLITM old 3rd*/ SZKITA /* 3rd Item Number - Kit String Generic Edit(25)*/
		,'       20001' SZMMCU /* Branch String Generic Edit(12)*/
		,IM2.SZITM SZITM /* Item Number - Short Numeric Generic Edit(8)*/
		,REQUIREMENT.PART_ID /* IM2.SZLITM   */ SZLITM /* 2nd Item Number String Generic Edit(25)*/
		,IM2.SZAITM SZAITM /* 3rd Item Number String Generic Edit(25)*/
		,'       20001' SZCMCU /* Component Branch String Generic Edit(12)*/
		,'' /* CAST(CAST(OP_SEQ_RANK.RANK AS VARCHAR) + CAST(PC_RANK.RANK AS VARCHAR) AS INTEGER)  */ SZCPNT /* Component Line Number Numeric Generic Edit(4)*/
		...
		,CAST(CASE 
				WHEN BOM_SIDE.SZOPSQ IS NULL
					OR LTRIM(RTRIM(BOM_SIDE.SZOPSQ)) = ''
					THEN 0.00 /* confirmed Bill wants blanks to be zero*/
				ELSE CAST(LTRIM(RTRIM(BOM_SIDE.SZOPSQ)) AS DECIMAL(4, 2)) * 100
				END AS INTEGER) SZOPSQ /* Sequence Number - Operations Numeric Generic Edit(5)*/
		...
		,dbo.JDEJulian(GETDATE()) SZUPMJ /* Date - Updated Date Generic Edit(6)*/
		...
		,'' SZBOSTR
	/* Prep Code String UDC (F30L PC)(4)*/ FROM REQUIREMENT REQUIREMENT
	...
	WHERE REQUIREMENT.WORKORDER_LOT_ID = '0'
		AND REQUIREMENT.WORKORDER_TYPE IN (
			'M'
			,'S'
			) /* M per Bill 8/7/2019, S per Bill 9/19/2019*/
		AND (
			REQUIREMENT.SUBORD_WO_SUB_ID IS NULL
			OR REQUIREMENT.SUBORD_WO_SUB_ID = 0
			)
		AND IMS.SZSTKT IN (
			'M'
			,'S'
