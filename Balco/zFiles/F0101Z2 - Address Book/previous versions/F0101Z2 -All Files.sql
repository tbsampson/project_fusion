SELECT
	...
	,COMP.SZEDBT
--	,COMP.SZEDTN
	,(ROW_NUMBER() OVER(ORDER BY	COMP.SZEDTN)) + 72000 SZEDTN
	,COMP.SZEDLN
	,COMP.SZEDCT
	,COMP.SZTYTN
	,COMP.SZEDFT
	...
	,COMP.SZTNAC
--	,COMP.SZAN8
	,(ROW_NUMBER() OVER(ORDER BY	COMP.SZAN8)) + 72000 SZAN8
	,COMP.SZALKY
	,COMP.SZTAX
	,COMP.SZALPH
	,COMP.SZDC
	...
	(		
		...
		*/
		SELECT 'JDE' SZEDUS
			...
			,'' SZPNID
			,'A' SZTNAC
			,CAST(72000 + CUSTOMER.ROWID AS INTEGER) SZAN8
		...
		*/
		SELECT 'JDE' SZEDUS
			...
			,CASE 
				WHEN CUST_ADDRESS.ADDR_1 IS NOT NULL
					/* Only use old ADDR_1 if it looks like a street address*/
		...
		LEFT JOIN (
			...
			FROM CUST_ADDRESS A
			WHERE A.ADDR_NO > 1
			) CB_LIST ON CB_LIST.CUSTOMER_ID = CUST_ADDRESS.CUSTOMER_ID
