...
,(
 ...
 ) + 74500	SZEDTN -- EDI - Transaction Number [Generic Edit] String (22)
...
/*,ISNULL(CASE 
		 ...
		 END,'')*/
,ISNULL(LEFT(LTRIM(RTRIM(CUSTOMER.ADDR_1)),40),'')	
...
/*,ISNULL(CASE 
			...
			ELSE ''
		 END,'')*/
,ISNULL(LEFT(LTRIM(RTRIM(CUSTOMER.ADDR_2)),40),'')
			SZADD2 -- Address Line 2 [Generic Edit] String (40)
,ISNULL(LEFT(LTRIM(RTRIM(CUSTOMER.ADDR_3)),40),'')
...
/*,ISNULL(CASE 
		 ...
		 END,'')*/
,ISNULL(LEFT(LTRIM(RTRIM(CUSTOMER.ZIPCODE)),12),'')
...
/*,ISNULL(CASE 
		 ...
		 END,'')*/
,ISNULL(LEFT(LTRIM(RTRIM(CUSTOMER.CITY)),25),'')
			SZCTY1 -- City [Generic Edit] String (25)
,CASE WHEN LEFT(CUSTOMER.ZIPCODE,5) BETWEEN '00000' AND '99999' THEN 'US' ELSE 'CA' END
			SZCTR -- Country [UDC (00 CN)] String (3) -- *** NOTE: Balco needs to change the international customers country (manually) after load ***
/*,ISNULL(CASE 
		 ...
		 END,'')*/
,CASE
	WHEN ISNULL(LEFT(LTRIM(RTRIM(CUSTOMER.STATE)),2),'') 	IN ('','*','AB','AI','AK','AL','AM','AR','AU','AZ','BC','CA','CN','CO','CT','CZ','DC','DE','DF','FL','GA','HI','IA','ID','IL','IN','KS','KY','LA','MA','MB','MD','ME','MI','MN','MO','MS','MT','MX','NB','NC','ND','NE','NF','NH','NJ','NL','NM','NS','NSW','NT','NU','NV','NY','NZ','OH','OK','ON','OR','PA','PE','PR','QC','RI','RJ','RS','SA','SC','SD','SK','SP','TAS','TN','TX','UT','VA','VI','VIC','VT','WA','WI','WV','WY','YT','ZZ')
		THEN ISNULL(LEFT(LTRIM(RTRIM(CUSTOMER.STATE)),2),'')
		ELSE ''
 END
...
FROM
	...
	SELECT
		 C.ROWID ROWID
		,C.ID CUSTOMER_ID
		,MAX(NC.ADDR_NO) MAX_ST_ADDR
		
	FROM CUSTOMER C
	JOIN _NEW_CUSTOMERS NC
		ON C.ID = NC.NEW_CUSTOMERS
