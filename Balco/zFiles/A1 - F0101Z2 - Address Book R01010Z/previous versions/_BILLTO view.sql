...
FROM (
	...
	LEFT JOIN _CSB_REF$ CSB_REF ON CUSLIST.CUSTOMER_ID = CSB_REF.CUSTOMER_ID

	UNION
	-- BillTo addresses with more than 1 shipping address
	...
	UNION
		...
		SELECT  -- Customers with AR balance but no order history
		 'JDE' SZEDUS
		,'BAL0001' SZEDBT /* this is load ref, can put what we want in here*/
		,'' SZEDTN
		,0 SZEDLN
		...
		,'' SZMLNM
		,'' SZMLN1
		,CASE WHEN LEFT(CUST_ADDRESS.ADDR_1,1) = '%' OR LEFT(CUST_ADDRESS.ADDR_1,3) = 'C/O' OR LEFT(CUST_ADDRESS.ADDR_1,4) = 'ATTN' OR CUST_ADDRESS.ADDR_1 IS NULL
...
UNION
		...
		SELECT  -- Customers not picked up with PRE_BILLTO
		 'JDE' SZEDUS
		,'BAL0001' SZEDBT /* this is load ref, can put what we want in here*/
		,'' SZEDTN
		,0 SZEDLN
...
AND ABT.SZALKY IS NULL
	...
	JOIN CUSTOMER CUSTOMER /* Bill To Addresses*/ ON CUSLIST.CUSTOMER_ID = CUSTOMER.ID
	LEFT JOIN CUST_ADDRESS CUST_ADDRESS /* Ship To Address*/ ON CUSLIST.CUSTOMER_ID = CUST_ADDRESS.CUSTOMER_ID
		AND CUST_ADDRESS.ADDR_NO = 1 	
