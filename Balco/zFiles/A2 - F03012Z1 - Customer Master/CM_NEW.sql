/*
	...
	IR 5/15/2019 Tom Sampson
 	-- BV R03010Z
*/
SELECT 'JDE' VOEDUS /* EDI - User ID String Generic Edit 10 (0)*/
	,'BALCM' VOEDBT /* EDI - Batch Number String Generic Edit (15)*/
	,ROW_NUMBER() OVER (
		ORDER BY CUSTOMER.ID
		)  + 6107 VOEDTN /* EDI - Transaction Number String Generic Edit (22)*/
	...
	,BTST.SZAN8 VOAN8 /* Address Number Numeric UDC (8) */
	...
	,CASE 
		WHEN CUSTOMER.TERMS_NET_DAYS = 0
			THEN 'NI'
		WHEN CUSTOMER.TERMS_NET_DAYS = 3
			THEN 'N20'
		WHEN CUSTOMER.TERMS_NET_DAYS = 20
			THEN 'N20'
		WHEN CUSTOMER.TERMS_NET_DAYS = 30
			THEN 'N3'
		WHEN CUSTOMER.TERMS_NET_DAYS = 35
			THEN 'N20'
		WHEN CUSTOMER.TERMS_NET_DAYS = 45
			THEN 'N4'
		WHEN CUSTOMER.TERMS_NET_DAYS = 60
			THEN 'N6'
		WHEN CUSTOMER.TERMS_NET_DAYS = 90
			THEN 'N9'
		WHEN CUSTOMER.TERMS_NET_DAYS = 301
			THEN 'N20'
		ELSE ''
	...
	,CASE 
		WHEN AB.SZAT1 IN (
			...
			THEN SUM(CASE 
						WHEN YEAR(RECEIVABLE.INVOICE_DATE) = 2019
							THEN CAST(RECEIVABLE.TOTAL_AMOUNT * 100 AS BIGINT)
						ELSE 0
						END)
		ELSE 0
	...
	,dbo.JDEJulian(CUSTOMER.OPEN_DATE) VODAOJ /* Date - Account Opened Date Generic Edit (6)*/
	...
	,'' VOGOPASF
/* GOP Arrival or Ship Flag Character UDC (34A AS) (1)*/
FROM _BT_ST_NEW BTST
...
JOIN _ADDRESS_BOOK_NEW AB
	ON BTST.SZAN8 = AB.SZAN8
GROUP BY BTST.SZAN8 /*	,CSB_REF.CSB*/
	,BTST.[TYPE]
	,AB.SZAT1 /*	,CSB_REF.CUSTOMER_ID*/
	,CUSTOMER.ID
	,CUSTOMER.TERMS_NET_DAYS
	,dbo.JDEJulian(CUSTOMER.OPEN_DATE)
