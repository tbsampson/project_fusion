/*
	F03012Z1 Customer Master - File 1 VER0002
...
,ROW_NUMBER() OVER(ORDER BY	_ADDRESS_BOOK.SZAN8)   VOEDTN   -- EDI - Transaction Number String Generic Edit (22)
...
,_ADDRESS_BOOK.SZAN8   VOAN8   -- Address Number Numeric UDC (8)
...
,CASE WHEN CUSTOMER.OPEN_DATE IS NOT NULL
...
,CASE
	...
	WHEN _ADDRESS_BOOK.SZAT1 = 'CS' THEN 'S'
...
FROM
	_ADDRESS_BOOK _ADDRESS_BOOK

LEFT JOIN CUSTOMER CUSTOMER -- Bill To Addresses
	ON 
	
	  CASE
		WHEN REPLACE(LEFT(REPLACE(_ADDRESS_BOOK.SZALKY,'BC_',''),CHARINDEX('_', REPLACE(_ADDRESS_BOOK.SZALKY,'BC_',''))),'_','') = ''
			THEN REPLACE(_ADDRESS_BOOK.SZALKY,'BC_','')
			ELSE REPLACE(LEFT(REPLACE(_ADDRESS_BOOK.SZALKY,'BC_',''),CHARINDEX('_', REPLACE(_ADDRESS_BOOK.SZALKY,'BC_',''))),'_','')
	  END 
	
	= CUSTOMER.ID

LEFT JOIN RECEIVABLE RECEIVABLE
	ON REPLACE(_ADDRESS_BOOK.SZALKY,'BC_','') = RECEIVABLE.CUSTOMER_ID
