...
SELECT
	 ...
	 SHIPTO.SZEDUS
	,SHIPTO.SZEDBT
	,(ROW_NUMBER() OVER(ORDER BY	SHIPTO.SORT)) + 76000 SZEDTN
	,SHIPTO.SZEDLN
	,SHIPTO.SZEDCT
	,SHIPTO.SZTYTN
	,SHIPTO.SZEDFT
	...
	,SHIPTO.SZPNID
	,SHIPTO.SZTNAC
	,(ROW_NUMBER() OVER(ORDER BY	SHIPTO.SORT)) + 76000 SZAN8
	,SHIPTO.SZALKY
	,SHIPTO.SZTAX
	,SHIPTO.SZALPH
	,SHIPTO.SZDC
...
FROM
	...
	(
		...
		JOIN _BILLTO BILLTO
			ON 'BC_' + CUST_ADDRESS.CUSTOMER_ID = BILLTO.SZALKY
		...
		LEFT JOIN CUSTOMER CUSTOMER 
			ON CUST_ADDRESS.CUSTOMER_ID = CUSTOMER.ID
		 
		WHERE YEAR(ORDERS.ORDER_DATE) >= 2015
	) SHIPTO

LEFT JOIN CUSTOMER CUSTOMER
	ON SHIPTO.CUSTOMER_ID = CUSTOMER.ID

JOIN _BILLTO BILLTO
	ON 'BC_' + CUSTOMER.ID = BILLTO.SZALKY
